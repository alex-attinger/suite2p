...
def extract_traces(f_in, cell_masks, neuropil_masks, batch_size=500):
    ...
    F_smooth = np.zeros((ncells, n_frames),np.float32)
    Fneu_smooth = np.zeros((ncells, n_frames),np.float32)
...
def extract_traces_from_masks(ops, cell_masks, neuropil_masks):
    ...
    with BinaryRWFile(Ly=ops['Ly'], Lx=ops['Lx'],
        ...
        F, Fneu,F_smooth,Fneu_smooth = extract_traces(f, cell_masks, neuropil_masks, batch_size=batch_size)
    if 'reg_file_chan2' in ops:
        with BinaryRWFile(Ly=ops['Ly'], Lx=ops['Lx'],
                          filename=ops['reg_file_chan2']) as f:    
            F_chan2, Fneu_chan2,_,_ = extract_traces(cell_masks, neuropil_masks, batch_size=batch_size)
    return F, Fneu, F_chan2, Fneu_chan2, F_smooth,Fneu_smooth
...
def extraction_wrapper(stat, f_reg, f_reg_chan2=None, cell_masks=None, neuropil_masks=None, ops=default_ops()):
    ...
    F, Fneu,F_smooth,Fneu_smooth = extract_traces(f_reg, cell_masks, neuropil_masks, batch_size=batch_size)
    if f_reg_chan2 is not None:
        F_chan2, Fneu_chan2,_,_ = extract_traces(f_reg_chan2, cell_masks, neuropil_masks, batch_size=batch_size)
    else:
        F_chan2, Fneu_chan2 = [], []
    ...
    return stat, F, Fneu, F_chan2, Fneu_chan2,F_smooth,Fneu_smooth
...
def create_masks_and_extract(ops, stat, cell_masks=None, neuropil_masks=None):
    ...
    with BinaryRWFile(Ly=Ly, Lx=Lx, filename=reg_file) as f_in,\
        ...
        stat, F, Fneu, F_chan2, Fneu_chan2,F_smooth,Fneu_smooth = extraction_wrapper(stat, f_in, 
